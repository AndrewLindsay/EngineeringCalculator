...
struct ProjectWorkspaceModel: Equatable {
    ...
    init(document: CalculationDocument) throws {
        guard document.kind == .project else {
            throw ProjectWorkspaceError.notAProject
        }
    ...
    mutating func addCalculation(_ calculation: SavedCalculation, now: Date = Date()) throws {
        guard !document.calculations.contains(where: { $0.id == calculation.id }) else {
            throw ProjectWorkspaceError.duplicateCalculationID(calculation.id)
        }
    ...
    mutating func duplicateCalculation(id: UUID, newID: UUID = UUID(), now: Date = Date()) throws -> UUID {
        guard !document.calculations.contains(where: { $0.id == newID }) else {
            throw ProjectWorkspaceError.duplicateCalculationID(newID)
        }
        guard let sourceIndex = document.calculations.firstIndex(where: { $0.id == id }) else {
            throw ProjectWorkspaceError.calculationNotFound(id)
        }
        let source = document.calculations[sourceIndex]
        var copy = SavedCalculation(
            id: newID,
            name: "\(source.name) Copy",
            calculatorID: source.calculatorID,
            calculatorSchemaVersion: source.calculatorSchemaVersion,
            createdAt: now,
            modifiedAt: now,
            inputs: source.inputs,
            outputs: source.outputs,
            assumptions: source.assumptions,
            validationMessages: source.validationMessages,
            notes: source.notes
        )
        // Ensure the explicit assignment remains obvious if SavedCalculation gains mutable
    ...
    mutating func moveCalculation(from source: Int, to destination: Int, now: Date = Date()) throws {
        guard document.calculations.indices.contains(source) else {
            throw ProjectWorkspaceError.invalidIndex(source)
        }
        guard destination >= 0 && destination <= document.calculations.count else {
            throw ProjectWorkspaceError.invalidIndex(destination)
        }
        ...
        for candidate in materials {
            ...
            if let existing = existingByID[candidate.id] {
                ...
                guard existingFingerprint == fingerprint else {
                    throw ProjectWorkspaceError.materialIDConflict(candidate.id)
                }
                continue
            }
...
enum ProjectWorkspaceError: Error, Equatable, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notAProject:
            return "This document is not an Engineering Calculator project."
        case .emptyName:
            return "Project and calculation names cannot be empty."
        case let .calculationNotFound(id):
            return "Calculation \(id.uuidString) was not found in the project."
        case let .duplicateCalculationID(id):
            return "Calculation UUID \(id.uuidString) already exists in the project."
        case let .invalidIndex(index):
            return "Project calculation index \(index) is invalid."
        case let .materialIDConflict(id):
            return "Material UUID \(id.uuidString) has a different engineering definition in this project."
        }
    }
}
