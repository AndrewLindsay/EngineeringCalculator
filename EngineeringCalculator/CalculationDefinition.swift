...
struct CalculationEquation: Identifiable, Hashable { let id:String;let title:String;let mathML:String;let explanation:String?;init(id:String,title:String,mathML:String,explanation:String?=nil){self.id=id;self.title=title;self.mathML=mathML;self.explanation=explanation} }
...
enum CalculationRegistry {
    static let all:[CalculationDefinition]=[
        ...
        .init(id:"pipeHeatTransfer",title:"Pipe Heat Transfer",subtitle:"Adaptive steady-state radial conduction through multilayer pipe and insulation",systemImage:"thermometer.medium",details:pipeHeatTransferDetails),
    ...
    private static let pipeHeatTransferDetails=CalculationDetails(
        overview:[
            "Calculates steady-state radial heat conduction through one or more concentric cylindrical physical layers.",
            "Thermal conductivity is resolved from the Material Library at local calculated temperatures. Missing or out-of-range conductivity data prevent a valid result.",
            "Temperature-dependent physical layers are adaptively subdivided into computational cells. Refinement continues where conductivity varies by more than 1% across a cell, subject to convergence and solver limits.",
            "The physical layers entered by the user are preserved in the reported results; computational cells are numerical subdivisions only."
        ],
        equations:[
            .init(id:"radialR",title:"Computational-cell radial resistance",mathML:#"<math><msub><mi>R</mi><mi>j</mi></msub><mo>=</mo><mfrac><mrow><mi>ln</mi><mfenced><mfrac><msub><mi>r</mi><mrow><mi>o</mi><mo>,</mo><mi>j</mi></mrow></msub><msub><mi>r</mi><mrow><mi>i</mi><mo>,</mo><mi>j</mi></mrow></msub></mfrac></mfenced></mrow><mrow><mn>2</mn><mi>π</mi><msub><mi>k</mi><mi>j</mi></msub><mi>L</mi></mrow></mfrac></math>"#,explanation:"k is resolved at the local mean temperature of each computational cell."),
            .init(id:"totalR",title:"Total conduction resistance",mathML:#"<math><msub><mi>R</mi><mi>total</mi></msub><mo>=</mo><munderover><mo>∑</mo><mrow><mi>j</mi><mo>=</mo><mn>1</mn></mrow><mi>N</mi></munderover><msub><mi>R</mi><mi>j</mi></msub></math>"#),
            ...
            .init(id:"cellDrop",title:"Cell temperature drop",mathML:#"<math><mi>Δ</mi><msub><mi>T</mi><mi>j</mi></msub><mo>=</mo><mi>Q</mi><msub><mi>R</mi><mi>j</mi></msub></math>"#,explanation:"Interface temperatures are reconstructed from the common heat rate and successive cell resistances.")
        ],
        variables:[
            .init(id:"ri",symbolMathML:#"<math><msub><mi>r</mi><mi>i</mi></msub></math>"#,definition:"Computational-cell inner radius",unit:"m"),
            .init(id:"ro",symbolMathML:#"<math><msub><mi>r</mi><mi>o</mi></msub></math>"#,definition:"Computational-cell outer radius",unit:"m"),
            .init(id:"k",symbolMathML:#"<math><mi>k</mi></math>"#,definition:"Local thermal conductivity resolved from the Material Library",unit:"W/(m·K)"),
        ...
        assumptions:[
            ...
            "Concentric circular cylindrical physical layers with uniform thickness.",
            "Inside and outside temperatures are solid boundary temperatures; convection films are not yet included.",
            "No axial conduction, contact resistance, radiation, internal heat generation or end effects.",
            "Each computational cell uses conductivity evaluated at its local arithmetic mean temperature. The nonlinear temperature/property field is solved iteratively with under-relaxation.",
            "Adaptive refinement uses equal log-radius subdivisions. Constant-property layers normally require one computational cell.",
            "Default refinement threshold is 1% relative conductivity variation across a cell; heat-rate refinement tolerance is 1×10⁻⁶; maximum refinement is 12 passes and 4096 cells per physical layer."
        ],
        references:[
            "Fourier heat conduction in cylindrical coordinates: R = ln(ro/ri)/(2πkL) for a constant-k radial interval.",
            "For temperature-dependent conductivity, the adaptive discretisation approximates the continuous relation Q ln(ro/ri) = 2πL ∫ k(T)dT as the computational cells are refined."
        ])
