...
struct HomeView: View {
                            ...
                            Label {
                                VStack(alignment: .leading, spacing: density.rowPadding) {
                                    Text(calculation.title)
                                        .font(.system(size: density.bodySize, weight: .semibold))
                                ...
                                Image(systemName: calculation.systemImage)
                                    .font(.system(size: density.bodySize + 3))
                            }
    ...
    private func move(from source: IndexSet, to destination: Int) {
        ...
        storedOrder = items.map(\.id).joined(separator: ",")
    }
}
...
private struct SettingsView: View {
    ...
    var body: some View {
        Form {
            Section("Interface") {
                Picker("Interface Density", selection: $densityRaw) {
                ...
                Text("Compact shows more engineering data on screen. Standard uses a conventional layout. Comfortable increases text and spacing.")
                    .font(.system(size: density.smallSize))
                    .foregroundStyle(.secondary)
            }
...
private struct PlaceholderCalculatorView: View {
    var body: some View {
        ContentUnavailableView(
            ...
            description: Text("Add future calculation modules here.")
        )
