...
enum MaterialComparisonEngine {
    ...
    static func compare(_ materials: [EngineeringMaterial], referenceMaterialID: UUID? = nil) -> MaterialComparison {
        ...
        func text(_ id: String, _ section: MaterialComparisonSection, _ label: String, _ value: (EngineeringMaterial) -> String?) { rows.append(makeRow(id, section, label, ordered) { value($0).map(MaterialComparisonValue.text) ?? .missing }) }
        func number(_ id: String, _ section: MaterialComparisonSection, _ label: String, _ unit: String?, _ value: (EngineeringMaterial) -> Double?) { rows.append(makeRow(id, section, label, ordered) { value($0).map { .number($0, unit: unit) } ?? .missing }) }
        func series(_ id: String, _ section: MaterialComparisonSection, _ label: String, _ value: (EngineeringMaterial) -> MaterialPropertySeries?) { rows.append(makeRow(id, section, label, ordered) { value($0).map(MaterialComparisonValue.propertySeries) ?? .missing }) }
        text("category",.identity,"Category",{ $0.category }); text("grade",.identity,"Grade",{ $0.grade }); text("unsDesignation",.identity,"UNS designation",{ $0.unsDesignation }); text("standardDesignation",.identity,"Standard designation",{ $0.standardDesignation }); text("productForm",.identity,"Product form",{ $0.productForm }); text("materialCondition",.identity,"Material condition",{ $0.materialCondition }); text("source",.identity,"Source",{ $0.source }); text("notes",.identity,"Notes",{ $0.notes })
        number("density",.physical,"Density","kg/m³",{ $0.densityKgM3 })
        number("thermalConductivity",.thermal,"Thermal conductivity","W/(m·K)",{ $0.thermalConductivityWMK }); series("thermalConductivityModel",.thermal,"Thermal conductivity model",{ $0.thermalConductivitySeries }); number("specificHeatCapacity",.thermal,"Specific heat capacity","J/(kg·K)",{ $0.specificHeatCapacityJkgK }); series("specificHeatCapacityModel",.thermal,"Specific heat capacity model",{ $0.specificHeatCapacitySeries }); number("thermalExpansion",.thermal,"Thermal expansion","µm/(m·K)",{ $0.thermalExpansionMicrostrainPerK }); series("thermalExpansionModel",.thermal,"Thermal expansion model",{ $0.thermalExpansionSeries })
        number("youngsModulus",.mechanical,"Young's modulus","GPa",{ $0.youngsModulusGPa }); series("youngsModulusModel",.mechanical,"Young's modulus model",{ $0.youngsModulusSeries }); number("poissonsRatio",.mechanical,"Poisson's ratio",nil,{ $0.poissonsRatio }); series("poissonsRatioModel",.mechanical,"Poisson's ratio model",{ $0.poissonsRatioSeries }); number("yieldStrength",.mechanical,"Yield strength","MPa",{ $0.yieldStrengthMPa }); series("yieldStrengthModel",.mechanical,"Yield strength model",{ $0.yieldStrengthSeries }); number("ultimateTensileStrength",.mechanical,"Ultimate tensile strength","MPa",{ $0.ultimateTensileStrengthMPa }); series("ultimateTensileStrengthModel",.mechanical,"Ultimate tensile strength model",{ $0.ultimateTensileStrengthSeries }); number("shearModulus",.mechanical,"Shear modulus","GPa",{ $0.shearModulusGPa }); series("shearModulusModel",.mechanical,"Shear modulus model",{ $0.shearModulusSeries }); number("compressiveStrength",.mechanical,"Compressive strength","MPa",{ $0.compressiveStrengthMPa }); number("smys",.mechanical,"SMYS","MPa",{ $0.smysMPa }); number("smts",.mechanical,"SMTS","MPa",{ $0.smtsMPa })
        number("electricalResistivity",.electrical,"Electrical resistivity","Ω·m",{ $0.electricalResistivityOhmM }); series("electricalResistivityModel",.electrical,"Electrical resistivity model",{ $0.electricalResistivitySeries })
        number("minimumServiceTemperature",.serviceLimits,"Minimum service temperature","°C",{ $0.minimumServiceTemperatureC }); number("maximumServiceTemperature",.serviceLimits,"Maximum service temperature","°C",{ $0.maximumServiceTemperatureC })
