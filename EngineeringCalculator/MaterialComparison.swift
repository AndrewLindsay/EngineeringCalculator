...
enum MaterialComparisonEngine {
    ...
    static func compare(_ materials: [EngineeringMaterial], referenceMaterialID: UUID? = nil) -> MaterialComparison {
        ...
        func textRow(_ id: String, _ section: MaterialComparisonSection, _ label: String, _ value: (EngineeringMaterial) -> String?) { rows.append(makeRow(id: id, section: section, label: label, materials: ordered) { value($0).map(MaterialComparisonValue.text) ?? .missing }) }
        func numberRow(_ id: String, _ section: MaterialComparisonSection, _ label: String, unit: String?, _ value: (EngineeringMaterial) -> Double?) { rows.append(makeRow(id: id, section: section, label: label, materials: ordered) { value($0).map { .number($0, unit: unit) } ?? .missing }) }
        func seriesRow(_ id: String, _ section: MaterialComparisonSection, _ label: String, _ value: (EngineeringMaterial) -> MaterialPropertySeries?) { rows.append(makeRow(id: id, section: section, label: label, materials: ordered) { value($0).map(MaterialComparisonValue.propertySeries) ?? .missing }) }
    ...
    private static func numericDifferences(reference: MaterialComparisonValue, current: MaterialComparisonValue) -> (absolute: Double?, percentage: Double?) { guard case let .number(a, _) = reference, case let .number(b, _) = current else { return (nil, nil) }; let delta = b - a; return (delta, approximatelyEqual(a, 0) ? nil : delta / a * 100) }
    private static func equivalent(_ lhs: MaterialComparisonValue, _ rhs: MaterialComparisonValue) -> Bool { switch (lhs, rhs) { case (.missing, .missing): return true; case let (.text(a), .text(b)): return a == b; case let (.number(a, ua), .number(b, ub)): return ua == ub && approximatelyEqual(a, b); case let (.propertySeries(a), .propertySeries(b)): return equivalent(a, b); default: return false } }
...
struct MaterialComparisonView: View {
    ...
    init(initialSelectionOrder: [UUID] = [], referenceID: UUID? = nil, showingComparison: Bool = false, standaloneWindow: Bool = false) {
        _selectedIDs = State(initialValue: Set(initialSelectionOrder)); _selectionOrder = State(initialValue: initialSelectionOrder); _referenceID = State(initialValue: referenceID ?? initialSelectionOrder.first); _showingComparison = State(initialValue: showingComparison); self.standaloneWindow = standaloneWindow
    }
    ...
    private var selectionContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack { Text("\(selectedIDs.count) selected").foregroundStyle(.secondary); Spacer(); Button("Compare (\(selectedIDs.count))") { beginComparison() }.buttonStyle(.borderedProminent).disabled(selectedIDs.count < 2) }
        }.padding(20)
    }
    ...
    private func comparisonContent(_ comparison: MaterialComparison) -> some View {
        VStack(spacing: 0) {
            ...
            Divider(); comparisonTable(comparison)
        }
    }
            ...
            LazyVStack(alignment: .leading, spacing: 0) {
                ...
                ForEach(MaterialComparisonSection.allCases) { section in
                    ...
                    if !sectionRows.isEmpty {
                        ...
                        ForEach(sectionRows) { row in comparisonRow(row) }
                    }
                }
            }
            .padding(12)
            .background(MacScrollOffsetReader(offsetX: $horizontalScrollOffset))
        }
    }

    private func comparisonHeader(_ comparison: MaterialComparison) -> some View {
        HStack(spacing: 0) {
            Text("Property").fontWeight(.semibold).frame(width: propertyColumnWidth, alignment: .leading).padding(8).background(.background).offset(x: horizontalScrollOffset).zIndex(3)
            if let reference = comparison.materials.first { materialHeader(reference, subtitle: "Reference").background(.background).offset(x: horizontalScrollOffset).zIndex(2) }
            ForEach(Array(comparison.materials.dropFirst())) { material in materialHeader(material, subtitle: "Compared") }
        }.background(.quaternary.opacity(0.35))
    }

    private func comparisonRow(_ row: MaterialComparisonRow) -> some View {
        HStack(alignment: .top, spacing: 0) {
            Text(row.label).frame(width: propertyColumnWidth, alignment: .leading).padding(8).background(.background).offset(x: horizontalScrollOffset).zIndex(3)
            if let referenceCell = row.cells.first { comparisonCell(referenceCell).background(.background).offset(x: horizontalScrollOffset).zIndex(2) }
    ...
    private func mobileComparisonTable(_ comparison: MaterialComparison) -> some View {
        ...
        return ScrollView([.horizontal, .vertical]) {
            LazyVStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 0) { Text("Property").fontWeight(.semibold).frame(width: propertyColumnWidth, alignment: .leading).padding(8); ForEach(comparison.materials) { material in materialHeader(material, subtitle: material.id == comparison.referenceMaterialID ? "Reference" : "Compared") } }.background(.quaternary.opacity(0.35))
                ForEach(MaterialComparisonSection.allCases) { section in let sectionRows = rows.filter { $0.section == section }; if !sectionRows.isEmpty { Text(section.rawValue).font(.headline).frame(maxWidth: .infinity, alignment: .leading).padding(8).background(.regularMaterial); ForEach(sectionRows) { row in HStack(alignment: .top, spacing: 0) { Text(row.label).frame(width: propertyColumnWidth, alignment: .leading).padding(8); ForEach(row.cells) { cell in comparisonCell(cell) } }.overlay(alignment: .bottom) { Divider() } } } }
            }.padding(12)
        }
    }

    private func materialHeader(_ material: EngineeringMaterial, subtitle: String) -> some View { VStack(alignment: .leading) { Text(material.name).fontWeight(.semibold); Text(subtitle).font(.caption2).foregroundStyle(.secondary) }.frame(width: materialColumnWidth, alignment: .leading).padding(8) }
    private func comparisonCell(_ cell: MaterialComparisonCell) -> some View { VStack(alignment: .leading, spacing: 3) { Text(display(cell.value)).font(.body.monospacedDigit()); if cell.differsFromReference { if let delta = cell.absoluteDifference { Text(differenceText(delta: delta, percentage: cell.percentageDifference, value: cell.value)).font(.caption.monospacedDigit()).foregroundStyle(.secondary) } else { Text("Changed").font(.caption).foregroundStyle(.secondary) } } }.frame(width: materialColumnWidth, alignment: .leading).frame(minHeight: 42, alignment: .topLeading).padding(8).background(cell.differsFromReference ? Color.accentColor.opacity(0.10) : Color.clear) }
    ...
    private func openMacComparisonWindow() {
        let root = MaterialComparisonView(initialSelectionOrder: selectionOrder, referenceID: referenceID, showingComparison: true, standaloneWindow: true).environmentObject(store)
        let controller = NSHostingController(rootView: root); let window = NSWindow(contentViewController: controller)
        window.title = "Compare Materials"; window.styleMask = [.titled, .closable, .miniaturizable, .resizable]; window.setContentSize(NSSize(width: 1200, height: 760)); window.minSize = NSSize(width: 760, height: 560); window.collectionBehavior.insert(.fullScreenPrimary); window.center(); window.makeKeyAndOrderFront(nil)
    }
    #endif
}

#if os(macOS)
private struct MacScrollOffsetReader: NSViewRepresentable {
    ...
    final class Coordinator {
        ...
        func attach(from view: NSView) {
            ...
            detach(); clipView = clip; clip.postsBoundsChangedNotifications = true
