...
struct MaterialImportExportView: View {
    ...
    var body: some View {
            ...
            Form {
                Section("Import") {
                    Button {
                        ...
                        Label("Import Materials…", systemImage: "square.and.arrow.down")
                    }
                    Text("Import a .ecmaterial or .ecmaterials file. Imported materials are added to My Materials; existing materials are not replaced.")
                        .font(.caption)
        ...
        .fileImporter(
            ...
            allowedContentTypes: [.engineeringCalculatorMaterial, .engineeringCalculatorMaterialLibrary, .json],
        ...
        .alert(item: $importSummary) { summary in
            Alert(title: Text("Import Complete"), message: Text(summary.message), dismissButton: .default(Text("OK")))
        }
        .alert("Material File Error", isPresented: Binding(
    ...
    private func importFile(_ url: URL) {
        ...
        do {
            ...
            let report = try store.importPortableData(data)
            importSummary = MaterialImportSummary(report: report)
        } catch {
            errorMessage = error.localizedDescription
        }
    }
    ...
    private func export(_ material: EngineeringMaterial) {
        do {
            exportDocument = MaterialPortableFileDocument(data: try store.exportData(for: material))
            exportContentType = .engineeringCalculatorMaterial
            exportFilename = safeFilename(material.name)
            showingExporter = true
        } catch {
            errorMessage = error.localizedDescription
        }
