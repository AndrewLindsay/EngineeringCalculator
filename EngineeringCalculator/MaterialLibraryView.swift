...
struct MaterialLibraryView: View {
        ...
        .confirmationDialog("Delete Material?", isPresented: Binding(
            ...
            if let material = materialPendingDeletion {
                Button("Delete \(material.name)", role: .destructive) {
                    store.delete(id: material.id); materialPendingDeletion = nil
                }
            }
    ...
    private func categoryHeader(_ category: String) -> some View {
        HStack { Text(category); Spacer(); Text("\(materials(in: category).count)").font(.caption.monospacedDigit()).foregroundStyle(.secondary) }
    ...
    private func dropTarget(_ category: String) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Drag normally to move a user material; hold Option while dragging to copy. Built-in materials are always copied.")
                    .font(.caption2).foregroundStyle(.tertiary)
            }
            Spacer()
        }
        .foregroundStyle(.secondary).padding(.horizontal, 14).padding(.vertical, 12)
        .frame(maxWidth: .infinity, minHeight: 52, alignment: .leading)
        .background(.quaternary.opacity(0.22), in: RoundedRectangle(cornerRadius: 8))
        .overlay { RoundedRectangle(cornerRadius: 8).stroke(.quaternary, style: StrokeStyle(lineWidth: 1, dash: [5, 4])) }
        .contentShape(Rectangle())
        .help("Move user materials by dragging. Hold Option (⌥) while dragging to copy. Built-in materials are always copied.")
    }
#endif
    ...
    private func materialMenu(_ material: EngineeringMaterial, currentCategory: String) -> some View {
        ...
        Menu("Copy to Category") {
            ForEach(store.categories, id: \.self) { target in
                Button(target) { store.duplicate(material, to: target) }
            }
        }
        if !material.isBuiltIn {
    ...
    private func handleDroppedMaterials(_ items: [String], to category: String) -> Bool {
        ...
        var handled = false
        for item in items {
            guard let id = UUID(uuidString: item), let material = store.allMaterials.first(where: { $0.id == id }) else { continue }
            if material.isBuiltIn || optionHeld { store.duplicate(material, to: category) }
            else { store.move(material, to: category) }
            handled = true
        }
        return handled
    }
    ...
    private func row(_ material: EngineeringMaterial) -> some View {
        VStack(alignment: .leading, spacing: density.rowPadding) {
            HStack {
                Text(material.name).font(.system(size: density.bodySize, weight: .semibold)); Spacer()
...
private struct MaterialDetailView: View {
            ...
            if let material {
                ScrollView {
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        HStack {
                            Button(material.isBuiltIn ? "Duplicate to My Materials" : "Duplicate") { store.duplicate(material) }
                            if !material.isBuiltIn {
                ...
                .confirmationDialog("Delete Material?", isPresented: $confirmingDelete, titleVisibility: .visible) {
                    Button("Delete \(material.name)", role: .destructive) { store.delete(id: material.id); dismiss() }
    ...
    private func propertyRow(_ key: String, _ value: Double?, unit: String) -> some View {
        HStack(alignment: .firstTextBaseline, spacing: 0) {
            Text(key).fontWeight(.semibold).foregroundStyle(.secondary).frame(width: 175, alignment: .leading)
            Divider().padding(.horizontal, 12)
            if let value { Text("\(value.formatted()) \(unit)").monospacedDigit() } else { Text("Not specified").foregroundStyle(.secondary) }
            Spacer()
        }.padding(.horizontal, 14).padding(.vertical, 10).overlay(alignment: .bottom) { Divider() }
    }
}
