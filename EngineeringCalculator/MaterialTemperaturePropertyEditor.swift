...
struct MaterialTemperaturePropertyEditor:View{
    ...
    private var equationEditor:some View{VStack(alignment:.leading,spacing:10){Picker("Equation type",selection:$draft.equationKind){ForEach(MaterialEquationKind.allCases){Text($0.rawValue).tag($0)}};Text(draft.equationKind.formula+"   where T is in °C").font(.callout).monospaced();switch draft.equationKind{case .polynomial:LazyVGrid(columns:[GridItem(.flexible()),GridItem(.flexible())],spacing:8){parameterRow("Coefficient a",symbol:"a",value:$draft.a);parameterRow("Coefficient b",symbol:"b",value:$draft.b);parameterRow("Coefficient c",symbol:"c",value:$draft.c);parameterRow("Coefficient d",symbol:"d",value:$draft.d)};case .linearReference:referenceEquationEditor;parameterRow("Slope",symbol:"k",value:$draft.slope);case .relativeLinear:referenceEquationEditor;parameterRow("Temperature coefficient",symbol:"α",value:$draft.temperatureCoefficient,unit:"K⁻¹")};temperatureRangeEditor;Toggle("Allow extrapolation outside valid range",isOn:$draft.allowsExtrapolation);traceabilityEditor}}
    ...
    private var temperatureRangeEditor:some View{VStack(spacing:8){parameterRow("Minimum temperature",symbol:"Tmin",value:$draft.minimumTemperatureC,unit:"°C");parameterRow("Maximum temperature",symbol:"Tmax",value:$draft.maximumTemperatureC,unit:"°C")}}
    ...
    private func parameterRow(_ label:String,symbol:String,value:Binding<String>,unit:String="")->some View{HStack(spacing:10){HStack(spacing:4){Text(label).foregroundStyle(.secondary);Text(symbol).font(.body.monospaced()).fontWeight(.semibold)}.frame(minWidth:180,maxWidth:.infinity,alignment:.leading);TextField(symbol,text:value).multilineTextAlignment(.trailing).textFieldStyle(.roundedBorder).frame(minWidth:90,idealWidth:120,maxWidth:150);if !unit.isEmpty{Text(unit).foregroundStyle(.secondary).frame(minWidth:55,alignment:.leading)}}}
}
