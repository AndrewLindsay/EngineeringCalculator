...
struct MaterialPortableDocument: Hashable, Codable {
    ...
    init(kind: MaterialPortableFileKind, materials: [EngineeringMaterial]) {
        self.format = kind.rawValue; self.formatVersion = Self.currentFormatVersion; self.materials = materials
    }
}
...
enum MaterialPortableCodec {
    ...
    static func merge(imported: [EngineeringMaterial], into existing: [EngineeringMaterial]) -> (materials: [EngineeringMaterial], report: MaterialImportReport) {
        ...
        for source in imported {
            ...
            if usedIDs.contains(material.id) { material.id = UUID(); report.regeneratedUUIDCount += 1 }
            ...
            material.isBuiltIn = false; usedIDs.insert(material.id); usedNames.insert(material.name.lowercased()); result.append(material)
            report.importedCount += 1; report.importedNames.append(material.name)
        }
    ...
    private static func uniqueName(_ requested: String, usedNames: Set<String>) -> String {
        ...
        var index=2; while usedNames.contains("\(requested) (Imported \(index))".lowercased()) { index += 1 }; return "\(requested) (Imported \(index))"
    }
    private static var encoder: JSONEncoder { let e=JSONEncoder(); e.outputFormatting=[.prettyPrinted,.sortedKeys]; return e }
}

/// Result of reconciling a portable calculation/project with the receiving material library.
/// Conflicts are reported but never automatically resolved or written into the library.
...
final class MaterialLibraryStore: ObservableObject {
    ...
    /// Reconciles a calculation/project's embedded materials against both built-in and user
    /// materials, imports only genuinely new UUIDs, and returns all conflicts to the caller.
    /// Reopening the same document is therefore idempotent and cannot bloat the library.
    @discardableResult
    func importNewMaterials(from document: CalculationDocument) throws -> PortableCalculationMaterialImportReport {
        ...
        for source in candidates {
            // Re-check identity immediately before insertion. This protects against accidental
            // duplicate insertion if this method is later called from a more complex workflow.
            guard !allMaterials.contains(where: { $0.id == source.id }) else { continue }
            var material = source
            material.isBuiltIn = false
            material.category = canonicalCategory(material.category)
            userMaterials.append(material)
            importedIDs.append(material.id)
            importedNames.append(material.name)
        }
