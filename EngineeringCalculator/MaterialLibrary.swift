...
// MARK: - Portable material interchange
...
struct MaterialPortableDocument: Hashable, Codable {
    ...
    init(kind: MaterialPortableFileKind, materials: [EngineeringMaterial]) {
...
struct MaterialImportReport: Equatable {
    var importedCount: Int = 0
    var regeneratedUUIDCount: Int = 0
    var renamedCount: Int = 0
    var importedNames: [String] = []
}

enum MaterialPortableCodec {
    static func encode(material: EngineeringMaterial) throws -> Data {
        try encoder.encode(MaterialPortableDocument(kind: .material, materials: [portableCopy(material)]))
    }

    static func encode(library materials: [EngineeringMaterial]) throws -> Data {
        try encoder.encode(MaterialPortableDocument(kind: .library, materials: materials.map(portableCopy)))
    }
    ...
    static func decode(_ data: Data) throws -> (kind: MaterialPortableFileKind, materials: [EngineeringMaterial]) {
        ...
        do { document = try JSONDecoder().decode(MaterialPortableDocument.self, from: data) }
        catch { throw MaterialImportError.malformedFile }
        ...
        guard document.formatVersion <= MaterialPortableDocument.currentFormatVersion else { throw MaterialImportError.unsupportedVersion(document.formatVersion) }
        guard document.formatVersion > 0 else { throw MaterialImportError.unsupportedVersion(document.formatVersion) }
    ...
    static func merge(imported: [EngineeringMaterial], into existing: [EngineeringMaterial]) -> (materials: [EngineeringMaterial], report: MaterialImportReport) {
        ...
        var usedNames = Set(existing.map { $0.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() })
        var report = MaterialImportReport()

        for source in imported {
            var material = portableCopy(source)
            if usedIDs.contains(material.id) {
                material.id = UUID()
                report.regeneratedUUIDCount += 1
            }
            let originalName = material.name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Imported Material" : material.name.trimmingCharacters(in: .whitespacesAndNewlines)
            material.name = uniqueName(originalName, usedNames: usedNames)
            if material.name != originalName { report.renamedCount += 1 }
            material.isBuiltIn = false
            usedIDs.insert(material.id)
            usedNames.insert(material.name.lowercased())
            result.append(material)
            report.importedCount += 1
            report.importedNames.append(material.name)
        }
        return (result, report)
    }

    private static func portableCopy(_ source: EngineeringMaterial) -> EngineeringMaterial {
        var copy = source
        copy.isBuiltIn = false
        return copy
    }

    private static func uniqueName(_ requested: String, usedNames: Set<String>) -> String {
        guard usedNames.contains(requested.lowercased()) else { return requested }
        var index = 2
        while usedNames.contains("\(requested) (Imported \(index))".lowercased()) { index += 1 }
        return "\(requested) (Imported \(index))"
    }
...
final class MaterialLibraryStore: ObservableObject {
    ...
    let builtInMaterials: [EngineeringMaterial] = [
        ...
        EngineeringMaterial(name: "UNS S32760 (SAF 32760+)", category: "Steel", densityKgM3: 7800,
                            grade: "SAF 32760+", thermalConductivityWMK: 15, specificHeatCapacityJkgK: 500,
                            thermalExpansionMicrostrainPerK: 13.0, youngsModulusGPa: 200, electricalResistivityOhmM: 0.8e-6,
                            source: "\(alleimaS32760Source) — \(alleimaS32760URL)",
                            notes: "High-alloy duplex (super duplex) stainless steel. Temperature-dependent values are manufacturer datasheet values. Published thermal-expansion values are mean coefficients over 30 °C to the stated upper temperature. Alleima notes that prolonged exposure above 250 °C can change the microstructure and reduce impact strength; this is not stored as a generic maximum service temperature.", isBuiltIn: true,
                            unsDesignation: "S32760", standardDesignation: "EN 1.4501; ASTM A479 / ASME SA-479; ASTM A276 / ASME SA-276", productForm: "Bar", materialCondition: "As covered by Alleima SAF 32760+ bar datasheet",
                            thermalConductivitySeries: MaterialPropertySeries(referenceValue: 15, referenceTemperatureC: 20, temperatureTable: [MaterialPropertyPoint(temperatureC: 20, value: 15)], source: "\(alleimaS32760Source) — \(alleimaS32760URL)", basis: "Thermal conductivity, W/(m·K). Datasheet publishes 15 W/(m·°C) at 20 °C."),
                            specificHeatCapacitySeries: MaterialPropertySeries(referenceValue: 500, referenceTemperatureC: 20, temperatureTable: [MaterialPropertyPoint(temperatureC: 20, value: 500)], source: "\(alleimaS32760Source) — \(alleimaS32760URL)", basis: "Specific heat capacity, J/(kg·K), published at 20 °C."),
                            thermalExpansionSeries: MaterialPropertySeries(referenceValue: 13.0, referenceTemperatureC: 100, temperatureTable: [MaterialPropertyPoint(temperatureC: 100, value: 13.0), MaterialPropertyPoint(temperatureC: 200, value: 13.5), MaterialPropertyPoint(temperatureC: 300, value: 14.0)], source: "\(alleimaS32760Source) — \(alleimaS32760URL)", basis: "Mean coefficient of thermal expansion, ×10⁻⁶/K, for published ranges 30-100, 30-200 and 30-300 °C respectively; temperature field is the range upper bound."),
                            youngsModulusSeries: MaterialPropertySeries(referenceValue: 200, referenceTemperatureC: 20, temperatureTable: [MaterialPropertyPoint(temperatureC: 20, value: 200), MaterialPropertyPoint(temperatureC: 100, value: 194), MaterialPropertyPoint(temperatureC: 200, value: 186), MaterialPropertyPoint(temperatureC: 300, value: 180)], source: "\(alleimaS32760Source) — \(alleimaS32760URL)", basis: "Modulus of elasticity, GPa (datasheet table is MPa ×10³)."),
                            electricalResistivitySeries: MaterialPropertySeries(referenceValue: 0.8e-6, referenceTemperatureC: 20, temperatureTable: [MaterialPropertyPoint(temperatureC: 20, value: 0.8e-6)], source: "\(alleimaS32760Source) — \(alleimaS32760URL)", basis: "Electrical resistivity, Ω·m; datasheet publishes 0.8 μΩ·m at 20 °C.")),
    ...
    var categories: [String] {
        ...
        var seen = Set<String>()
        return names.compactMap { raw in let value=raw.trimmingCharacters(in:.whitespacesAndNewlines); guard !value.isEmpty else{return nil}; let key=value.lowercased(); guard seen.insert(key).inserted else{return nil}; return value }.sorted { $0.localizedCaseInsensitiveCompare($1) == .orderedAscending }
    }
    ...
    @discardableResult func importPortableData(_ data: Data) throws -> MaterialImportReport {
        let decoded = try MaterialPortableCodec.decode(data)
        let merged = MaterialPortableCodec.merge(imported: decoded.materials, into: userMaterials)
        userMaterials = merged.materials.map { var copy=$0; copy.category=canonicalCategory(copy.category); return copy }
        save()
