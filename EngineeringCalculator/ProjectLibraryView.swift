...
struct ProjectLibraryView: View {
    ...
    var body: some View {
        List {
            Section {
                Button { newProjectName = ""; showingNewProject = true } label: {
        ...
        .navigationDestination(isPresented: Binding(
            get: { pendingNewProject != nil },
            set: { if !$0 { pendingNewProject = nil } }
        )) {
        ...
        .alert("New Project", isPresented: $showingNewProject) {
            ...
            Text("Enter a project name. This name is stored inside the .ecproject file and is independent of the filename.")
        }
        .alert("Rename Project", isPresented: Binding(
            ...
            Button("Rename") { renameProjectFromLibrary() }
                .disabled(renameText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
        } message: {
            Text("Renaming changes the project title stored inside the project. It does not rename the .ecproject file.")
        }
    ...
    private func renameProjectFromLibrary() {
        ...
        do {
            ...
            try CalculationDocumentFileIO.data(for: updated).write(to: entry.fileURL, options: .atomic)
            projectLibrary.register(document: updated, at: entry.fileURL)
        } catch {
