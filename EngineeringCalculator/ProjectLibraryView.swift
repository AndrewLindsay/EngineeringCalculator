...
struct ProjectLibraryView: View {
    ...
    var body: some View {
        List {
            Section {
                Button { newProjectName = ""; showingNewProject = true } label: { Label("New Project…", systemImage: "plus.rectangle.on.folder") }.help("Create a new engineering project")
                Button { showingImporter = true } label: { Label("Open / Import Project…", systemImage: "folder.badge.plus") }.help("Open or add an existing .ecproject file to the Project Library")
            }
            Section("Projects") {
        ...
        .navigationDestination(isPresented: Binding(get: { pendingExistingEntry != nil }, set: { if !$0 { pendingExistingEntry = nil } })) { if let pendingExistingEntry { CataloguedProjectLoaderView(entry: pendingExistingEntry) } }
        .alert("New Project", isPresented: $showingNewProject) { TextField("Project name", text: $newProjectName); Button("Cancel", role: .cancel) { }; Button("Create") { createProject() }.disabled(newProjectName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) } message: { Text("Enter a project name. This name is stored inside the .ecproject file and is independent of the filename.") }
        .alert("Rename Project", isPresented: Binding(get: { renameEntry != nil }, set: { if !$0 { renameEntry = nil } })) { TextField("Project name", text: $renameText); Button("Cancel", role: .cancel) { renameEntry = nil }; Button("Rename") { renameProjectFromLibrary() }.disabled(renameText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) } message: { Text("Renaming changes the project title stored inside the project. It does not rename the .ecproject file.") }
        .alert("Project Already in Library", isPresented: Binding(get: { duplicateImport != nil }, set: { if !$0 { duplicateImport = nil } })) { Button("Open Existing Project") { openExistingDuplicate() }; Button("Create Independent Copy") { createIndependentCopy() }; Button("Cancel", role: .cancel) { duplicateImport = nil } } message: { Text("This file has the same project identity as ‘\(duplicateImport?.entry.title ?? "")’. Opening it again would create two references to the same project. Open the existing project or create a new independent copy with its own project identity.") }
