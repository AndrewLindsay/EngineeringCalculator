...
struct PipeWeightBuoyancyView: View {
    ...
    private var pipeGeometrySection: some View {
    ...
    private var additionalLayersSection: some View {
        Section {
    ...
    @ViewBuilder private func layerRow(_ layer: Binding<EditableLayer>) -> some View {
        ...
        let density = layer.wrappedValue.material.densityKgM3
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                VStack(alignment: .leading) {
                    Text(layer.wrappedValue.material.name).font(.headline)
                    if let density { Text("\(density, format: .number.precision(.fractionLength(0))) kg/m³").font(.caption).foregroundStyle(.secondary) }
                    else { Label("Density missing", systemImage: "exclamationmark.triangle.fill").font(.caption).foregroundStyle(.red) }
                }
                Spacer()
                Button(role: .destructive) { deleteLayer(id: id) } label: { Image(systemName: "trash") }.buttonStyle(.borderless)
            }
            numericField("Thickness (mm)", value: layer.thicknessMM)
            HStack {
                Button { moveLayer(id: id, offset: -1) } label: { Label("Move Up", systemImage: "arrow.up") }.disabled(isFirst(id))
                Button { moveLayer(id: id, offset: 1) } label: { Label("Move Down", systemImage: "arrow.down") }.disabled(isLast(id))
    ...
    private var layerTable: some View {
        ScrollView(.horizontal) {
            Grid(alignment: .trailing, horizontalSpacing: 10, verticalSpacing: 5) {
                ...
                if let result {
                    ForEach(Array(result.layers.enumerated()), id: \.element.id) { i, l in
                        GridRow { Text("\(i+1)"); Text(l.name).frame(minWidth: 90, alignment: .leading); tableNumber(l.innerDiameterM*1000,digits:2); tableNumber(l.thicknessM*1000,digits:2); tableNumber(l.outerDiameterM*1000,digits:2); tableNumber(l.densityKgM3,digits:0); tableNumber(l.areaM2,digits:5); tableNumber(l.massKgPerM,digits:2) }.font(.caption2)
                    }
                } else {
                    ForEach(Array(layers.enumerated()), id: \.element.id) { i, l in invalidLayerGridRow(index: i, layer: l) }
                }
            }.monospacedDigit().padding(.vertical, 4)
        }
    }

    private func invalidLayerGridRow(index: Int, layer: PipeLayer) -> some View {
        let densityText = layer.material.densityKgM3.map { $0.formatted(.number.precision(.fractionLength(0))) } ?? "Missing"
        return GridRow {
            Text("\(index + 1)")
            ...
            Text("—")
            Text(densityText).foregroundStyle(layer.material.densityKgM3 == nil ? Color.red : Color.primary)
            Text("—")
            Text("—")
        }.font(.caption2)
    }
    ...
    @ViewBuilder private func numericField(_ title: String, value: Binding<Double>) -> some View {
        LabeledContent(title) { TextField(title, value: value, format: .number.precision(.fractionLength(0...4))).multilineTextAlignment(.trailing)
#if os(iOS)
...
private struct AddPipeLayerView: View {
    ...
    var body: some View {
        Form {
            Section("Material") {
                Picker("Material", selection: $selectedMaterialID) {
                    ...
                    ForEach(store.categories, id: \.self) { category in Section(category) { ForEach(materials(in: category)) { material in Text(material.name).tag(Optional(material.id)) } } }
                }
                if let material = selectedMaterial {
                    LabeledContent("Category", value: material.category)
                    LabeledContent("Density") {
                        if let density = material.densityKgM3 { Text("\(density.formatted()) kg/m³") }
                        else { Label("Missing — calculation will be blocked", systemImage: "exclamationmark.triangle.fill").foregroundStyle(.red) }
                    }
                }
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .confirmationAction) { Button("Add") { guard let material=selectedMaterial else{return}; onAdd(material,max(0,thicknessMM)); dismiss() }.disabled(selectedMaterial == nil || thicknessMM <= 0) }
        }
