...
struct PipeWeightBuoyancyView: View {
    ...
    private var pipeConstruction: PipeConstruction {
        PipeConstruction(name: "Current Pipe", internalDiameterM: internalDiameterM, layers: layers, internalFluid: FluidDefinition(name: "Internal Fluid", densityKgM3: max(0, internalDensity)), externalFluid: FluidDefinition(name: "External Fluid", densityKgM3: max(0, externalDensity)))
    }
    ...
    private func loadStandaloneCalculation(_ document: CalculationDocument) throws -> String {
        ...
        extraLayers = construction.layers.dropFirst().map { EditableLayer(id: $0.id, material: $0.material, thicknessMM: $0.thicknessM * 1000) }
    ...
    private func makeCurrentDocument(name: String) throws -> CalculationDocument {
        guard let result else { throw PipeWeightBuoyancyPersistence.RestoreError.missingInput("result") }
        return try PipeWeightBuoyancyPersistence.makeDocument(name: name, construction: pipeConstruction, result: result)
    }

    private func updateProjectCase() {
        guard let context = projectUpdateContext else { return }
        do {
            try context.update(try makeCurrentDocument(name: context.calculationName))
            projectUpdateMessage = "\(context.calculationName) has been updated in the current project. Save the project to persist the change to its .ecproject file."
    ...
    @ViewBuilder private var pipeGeometrySection: some View {
        Section(String(localized: "pipeWeight.section.geometry")) {
            ...
            Picker("Pipe material", selection: Binding(get: { selectedPipeMaterialID ?? pipeMaterial?.id }, set: { newID in selectedPipeMaterialID = newID; if newID != openedPipeMaterial?.id { openedPipeMaterial = nil } })) {
                if let openedPipeMaterial, !materialStore.steelMaterials.contains(where: { $0.id == openedPipeMaterial.id }) { Text("\(openedPipeMaterial.name) (embedded)").tag(Optional(openedPipeMaterial.id)) }
    ...
    @ViewBuilder private var materialRequirementsSection: some View {
        if !materialValidation.canCalculate {
            Section("Material Data Required") { ForEach(materialValidation.issues) { issue in materialIssueRow(issue) }; Text("Every solid layer requires density before this calculation can run. Edit the material in the Material Library or choose another material.").font(.caption).foregroundStyle(.secondary) }
        } else {
    ...
    private var detailedLayerGrid: some View {
        Grid(alignment: .trailing, horizontalSpacing: 5) {
            ...
            if let result { ForEach(Array(result.layers.enumerated()), id: \.element.id) { index, layer in GridRow { Text("\(index + 1)"); Text(layer.name).frame(minWidth: 90, alignment: .leading); tableNumber(layer.innerDiameterM * 1000, digits: 2); tableNumber(layer.thicknessM * 1000, digits: 2); tableNumber(layer.outerDiameterM * 1000, digits: 2); tableNumber(layer.densityKgM3, digits: 0); tableNumber(layer.areaM2, digits: 5); tableNumber(layer.massKgPerM, digits: 2) } } } else { GridRow { Text("Calculation unavailable").foregroundStyle(.secondary).gridCellColumns(8) } }
        }.font(.caption2)
    }
