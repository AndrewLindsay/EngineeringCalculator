...
struct PipeWeightBuoyancyView: View {
    ...
    private var materialValidation: PipeWeightMaterialValidation {
        PipeWeightBuoyancyCalculator.validateMaterials(construction: pipeConstruction)
    }

    private var result: PipeWeightBuoyancyResult? {
        materialValidation.canCalculate ? PipeWeightBuoyancyCalculator.calculate(construction: pipeConstruction) : nil
    }
    ...
    var body: some View {
        Form {
            ...
            Section {
                DisclosureGroup("Calculation Details", isExpanded: $showDetails) {
                    if let d = CalculationRegistry.definition(id: "pipeWeightBuoyancy")?.details {
                        CalculationDetailsView(details: d)
                    }
                }
            }
        }
        ...
        .sheet(isPresented: $showingAddLayer) {
            NavigationStack {
                AddPipeLayerView { m, t in
                    extraLayers.append(EditableLayer(material: m, thicknessMM: t))
                }
    ...
    private func moveLayer(id: UUID, offset: Int) {
        ...
        extraLayers.swapAt(index, destination)
    }
